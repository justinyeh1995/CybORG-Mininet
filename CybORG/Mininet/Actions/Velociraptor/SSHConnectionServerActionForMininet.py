...
class SSHConnectionServerAction(RunProcessAction):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            hostname=hostname,
            command=f"/home/ubuntu/justinyeh1995/CASTLEGym/castle.2.venv/bin/python3 {path}/Mininet/systems/scripts/SSHConnectionServer.py -n {self.connection_key} " + \
    ...
    def execute(self, state: Union[State, None]) -> Observation:
        ...
        if random.random() >= self.success_probility:
           print ("The random number >= the success probility")
