...
class ShellRelayer:
    ...
    def get_bytes(self):
        new_bytes = self.shell_socket.recv(4096)
        if len(new_bytes) == 0:
    ...
    def deploy_marker(self):
        ...
        pos = -1
        while pos < 0:
            self.shell_output_bytes += self.get_bytes()
    ...
    def run(self, shell_socket, shutdown_message="ERROR"):
        ...
        while True:
            ...
            try:
                ...
                local_shutdown_message = "ERROR" if isinstance(e, DisconnectException) else shutdown_message

                self.shell_socket = None

                try:
                    self.send_response(local_shutdown_message)
                    self.unix_socket.close()
                except:
                    pass
